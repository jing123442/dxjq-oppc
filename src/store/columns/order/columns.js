...
const columns = {
  order: [
    ...
    { field: 'amountTotal', name: '订单总金额', width: 90, hide: true, detail: { type: 'span', isDisabled: true, style: 'width: 85%', serial: 9, util: '元', value: function(row) { return Number(row.amount) + Number(row.discountAmount) } } },
  ...
  fillerOrder: [
    ...
    { field: 'period', name: '', hide: true, search: { type: 'date-picker', model: 'month', placeholder: '请选择账期', value: (function() { const dateObj = formatPeriodDate(new Date()); return (dateObj.periodYear + '-' + dateObj.periodMonth + '-' + '01 00:00:00') }()) } },
