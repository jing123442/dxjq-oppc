<template>
  <div class="template-main">
    ...
    <el-dialog title="批量导入物流用户" :visible.sync="dialogExportCarVisible" :width="add_edit_dialog" :append-to-body="true">
...
import { $resetPassword, $importCarrierDownloadFile, $exportCarrierUserFile, $userOrgList, $userDriverDetailInfo, $userFindBaseDetail } from '@/service/user'
...
export default {
  ...
  data() {
    return initVueDataOptions(this, {
      ...
      queryCustURL: {
        ...
        list: {
          url: 'user/user/page_list_carrier',
          method: 'post',
          parse: {
  ...
  computed: {
    ...mapGetters({
      ...
      page_truck_column: 'common_truck_carrier_column',
  ...
  methods: {
    async onListEvent(type, row) {
      if (type === 'reset_pwd') {
        ...
        if (row.baseRole === 'driver') {
          const driverInfo = await $userDriverDetailInfo({ id: row.userId }).then(response => {
            return response.data || {}
          })
          // 用户基本信息
          this.detailRow = Object.assign({}, this.detailRow, driverInfo.user)
          // 驾驶证信息
          if (driverInfo.driverLicense) {
            this.detailRow = Object.assign({}, this.detailRow, driverInfo.driverLicense)
          }
          // 用户认证信息
          if (driverInfo.userInfo) {
            this.detailRow = Object.assign({}, this.detailRow, driverInfo.userInfo)
          }
          this.page_curr_detail_column = [...this.page_detail_column, ...this.page_truck_column]
        } else {
