...
const columns = {
  ...
  supplier: [
    ...
    { field: 'afterSalesPhone', name: '售后电话', hide: true, show: { type: 'text', ou: 2, placeholder: '请输入售后电话' }, detail: { type: 'span', serial: 8, ou: 2 } },
  ...
  goods: [
    ...
    { field: 'goodsBrand', name: '商品品牌', show: { type: 'text', ou: 2, placeholder: '请输入商品品牌' }, detail: { type: 'span', serial: 7, ou: 2 } },
    ...
    { field: 'linePrice', name: '划线单价(元)', hide: true,
      show: { type: 'text', ou: 3, placeholder: '请输入划线单价' },
      detail: { type: 'span', serial: 10, ou: 3 },
      rules: [{ required: true, message: '请输入划线单价', trigger: 'blur' }, { validator: 'validateMoneyNumber', message: '格式错误，请检查', trigger: 'blur' }] },
    { field: 'exchangePrice', name: '兑换价(积分)',
      show: { type: 'text', ou: 3, placeholder: '请输入兑换价(积分)' },
      detail: { type: 'span', serial: 11, ou: 3 },
      rules: [{ required: true, message: '请输入兑换价', trigger: 'blur' }, { validator: 'isValidateNumber', message: '格式错误，请检查', trigger: 'blur' }] },
    { field: 'inventory', name: '库存数量',
      show: { type: 'text', ou: 5, placeholder: '请输入库存数量' },
      detail: { type: 'span', serial: 11, ou: 4 },
      rules: [{ required: true, message: '请输入库存数量', trigger: 'blur' }, { validator: 'isValidateNumber', message: '格式错误，请检查', trigger: 'blur' }] },
    { field: 'description', name: '商品描述', hide: true, xs: 24, sm: 24, md: 24, lg: 24, xl: 24, nameSpan: 2,
      show: { type: 'ueditor', style: 'width: 98%', ou: 6, config: file.state.fileUEditorConfig, placeholder: '请输入商品描述' },
      detail: { type: 'span', model: 'html', style: 'width: 97%;overflow: hidden;display: block;', ou: 6 },
      rules: [{ required: true, message: '请输入商品描述', trigger: 'blur' }] },
    { field: 'goodsStatus', name: '商品状态', formatter: 'goodsStatus',
      show: { type: 'radio', ou: 0, obj: 'goodsStatus', placeholder: '商品状态', value: '0' },
      search: { type: 'select', obj: 'goodsStatus', placeholder: '商品状态' },
      detail: { type: 'radio', isDisabled: true, obj: 'goodsStatus', serial: 0, ou: 0 },
      rules: [{ required: true, message: '请选择商品状态', trigger: 'blur' }] },
    { field: 'imageStatus', name: '图片状态', formatter: 'imageStatus',
      search: { type: 'select', obj: 'imageStatus', placeholder: '图片状态' },
      rules: [{ required: true, message: '请选择商品状态', trigger: 'blur' }] },
  ...
  pointsAccount: [
    ...
    { field: 'mobile', name: '用户手机号',
      search: { type: 'text', dataType: 'number', placeholder: '请输入用户手机号' } },
    { field: 'userName', name: '用户姓名' },
    { field: 'accountId', name: '积分账户号' },
    { field: 'status', name: '账户状态', formatter: 'pointsAccoutStatus',
      search: { type: 'select', obj: 'pointsAccoutStatus', placeholder: '账户状态' } },
    ...
    { field: 'balance', name: '积分余额',
      search: { type: 'input-range', startPlaceholder: '请输入积分余额起始值', endPlaceholder: '请输入积分余额终止值' } },
    { field: 'useropts', stype: 'opt', ispush: false, name: '操作', fixed: 'right', width: 200, list: item => pointsAccoutOpt(item) }
  ],
  pointsAccountLog: [
    { field: 'id', fixed: 'left', name: '流水号',
      search: { type: 'text', dataType: 'number', placeholder: '请输入流水号'} },
    { field: 'type', name: '交易类型', formatter: 'accountLogType',
      search: { type: 'select', obj: 'accountLogType', placeholder: '交易类型' } },
    { field: 'note', name: '摘要' },
    { field: 'changeType', name: '变动类型' },
    { field: 'changeAmount', name: '发生额' },
    { field: 'accountBalance', name: '交易后余额' },
    { field: 'createDate', name: '交易时间', stype: 'format', formatFun: 'formateTData all',
  ...
  pointChange: [
    ...
    { field: 'changeType', name: '调整类型', ispush: false, formatter: 'changeType',
      show: { type: 'select', obj: 'changeType', placeholder: '请选择调整类型' },
      rules: [{ required: true, message: '请选择调整类型', trigger: 'blur' }] },
    { field: 'changeAmount', name: '调整数量',
      show: { type: 'text', placeholder: '请输入调整数量' },
      rules: [{ required: true, message: '请输入调整数量', trigger: 'blur' }, { validator: 'validateInputValueNumber', option: { min: 0, max: 999999 }, message: '范围必须在0-999999之间', trigger: 'blur' }] },
    { field: 'note', name: '备注', show: { type: 'textarea' } }
  ],
  inviteRecords: [
    { field: 'inviterMobile', fixed: 'left', name: '邀请人手机号',
      search: { type: 'text', dataType: 'number', placeholder: '请输入邀请人手机号' } },
    { field: 'inviteeMobile', name: '被邀请人手机号',
      search: { type: 'text', dataType: 'number', placeholder: '请输入被邀请人手机号' } },
    { field: 'inviteeRegisterDate', name: '被邀请人注册时间', stype: 'format', formatFun: 'formateTData all',
      search: { type: 'date-picker', placeholder: '' } }
  ]
}
