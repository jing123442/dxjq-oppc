<template>
  <div class="template-main">
    ...
    <!-- 导入 -->
    <el-dialog title="批量导入订单" :visible.sync="dialogExportVisible" :width="add_edit_dialog" :append-to-body="true">
...
export default {
  ...
  data() {
    return initVueDataOptions(this, {
      queryCustURL: {
        list: {
          // 后台配置
          url: 'settle/gway_gasorder/list_orders',
          method: 'post',
          parse: {
        ...
        edit: {
          url: 'settle/gway_gasorder/update_gasorder',
          method: 'post'
        },
        del: {
      ...
      flag: '',
      statusBtnList: [
        { type: 'plain', icon: '', event: 'all', name: '全部订单', flag: null },
        { type: 'plain', icon: '', event: 'day', name: '日常订单', flag: 0 },
        { type: 'plain', icon: '', event: 'sell', name: '线下直销', flag: 1 }
      ],
      buttonsList: [
  ...
  computed: {
    ...mapGetters({
      mode_list: 'order_self_mode_list',
      page_status: 'order_self_page_status',
      page_column: 'order_self_column',
      select_list: 'order_self_select_list',
  ...
  methods: {
    ...
    initTotalData() {
      $sumData({
        ...
        url: 'settle/gway_gasorder/get_sum_total_new'
      }).then(res => {
    ...
    // 模板下载
    downloadModel() {
      const url = 'settle/gway_gasorder/download_gasorder_tpl'
      $importDownloadFile(url, { orgId: this.importRow.orgId, times: (new Date()).getTime() }).then(response => {
        const fileName = 'gasorder_tpl-' + this.importRow.orgId + '.xlsx'
    ...
    btnClickEvent(btnObj, row) {
      if (btnObj.type === 'ok') {
        ...
        const url = 'settle/gway_gasorder/import_gasorder'
