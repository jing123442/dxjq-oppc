...
import { $importReceivFile } from '@/service/information'
...
export default {
  ...
  data() {
    return initVueDataOptions(this, {
      ...
      queryCustURL: {
        list: {
          url: 'information/lng_pick_up_price/list',
          method: 'post',
          parse: {
  ...
  computed: {
    ...mapGetters({
      page_column: 'information_receiv_column',
      mode_list: 'information_receiv_mode_list',
      page_status: 'information_receiv_page_status',
      select_list: 'information_receiv_select_list',
  ...
  methods: {
    ...
    btnClickEvent(btnObj, row) {
      if (btnObj.type === 'ok') {
        ...
        $importReceivFile({ file: _fromData, headers: this.headers }).then(response => {
