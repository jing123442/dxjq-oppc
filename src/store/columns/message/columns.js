...
const columns = {
  message: [
    ...
    { field: 'publishTime', name: '发布时间', formatFun: 'formateTData all', stype: 'format', show: { type: 'date-picker', serial: 8, parent: 'mcMessage', format: 'yyyy-MM-dd HH', valueFormat: 'timestamp', model: 'datetime', placeholder: '选择发布时间', timeOptions: timeOptions, class: 'date-time-picker' }, detail: { type: 'span', isDisabled: true, serial: 3, formatFun: 'formateTData all', stype: 'format' } },
    ...
    { field: 'clientRoleTableList', stype: 'list-tag', ispush: false, porps: { name: 'title' }, name: '接收角色', show: { type: 'hide', serial: 20 }, detail: { type: 'span', stype: 'format', formatFun: 'roleList', isDisabled: true, serail: 5, obj: 'roleList' } },
    { field: 'clientRoleList', hide: true, stype: 'list-tag', porps: { name: 'title' }, name: '接收角色', show: { type: 'select', mulField: { clientId: 1, roleId: 0 }, serial: 3, multiple: true, obj: 'roleList', placeholder: '请选择要接收的角色' }, rules: [{ required: true, validator: roleSelectRules, trigger: 'focus' }] },
