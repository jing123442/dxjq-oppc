...
const columns = {
  customer: [
    ...
    { field: 'catalogsId', name: '分类', hide: true, show: { type: 'cascader', serial: 2, ou: 1, iType: 'string', parent: 'qaContent', mulField: { parentId: 0, catalogsId: 1 }, props: { lazy: true, value: 'id', label: 'name', lazyLoad (node, resolve) { utilsQATreeList(node, resolve) } }, style: 'width: 95%', placeholder: '请输入排序' }, detail: { type: 'span', model: 'cascader', serial: 2, ou: 1, iType: 'string', mulField: { parentCatalogsName: 0, catalogsName: 1 } }, rules: [{ required: true, message: '请输入分类！', trigger: 'change' }] },
