...
const columns = {
  ...
  type: [
    ...
    { field: 'sortOrder', name: '排序', fixed: 'left', show: { type: 'text', serial: 2, style: 'width: 95%', placeholder: '请输入排序' }, 'show-child': { type: 'text', serial: 2, style: 'width: 95%', placeholder: '请输入排序' }, detail: { type: 'span', serial: 2 }, 'detail-child': { type: 'span', serial: 2 }, rules: [{ required: true, message: '请输入正确的排序，只能为数字！', trigger: 'blur' }, { validator: 'isValidateNumber', message: '请输入正确的排序，只能为数字！', trigger: 'blur' }] },
    ...
    { field: 'useropts', stype: 'opt', ispush: false, name: '操作', fixed: 'right', width: 260, list: function(scope) { return scope.row.parentId === 0 ? [{ type: 'add-child', name: '新建子分类' }, { type: 'edit', name: '编辑' }, { type: 'del', name: '删除' }, { type: 'details', name: '详情' }] : [{ type: 'edit-child', name: '编辑' }, { type: 'del-child', name: '删除' }, { type: 'detail-child', name: '详情' }] } }
  ]
}
