...
const columns = {
  ...
  supplier: [
    ...
    { field: 'userPhone', name: '联系人电话', show: { type: 'number', precision: null, ou: 2, placeholder: '请输入联系人电话' }, rules: [{ required: true, message: '请输入联系人电话', trigger: 'blur' }], detail: { type: 'span', serial: 5, ou: 2 } },
    ...
    { field: 'afterSalesPhone', name: '售后电话', hide: true, show: { type: 'number', precision: null, ou: 2, placeholder: '请输入售后电话' }, detail: { type: 'span', serial: 8, ou: 2 } },
    { field: 'bank', name: '开户银行', hide: true, show: { type: 'text', ou: 3, placeholder: '请输入开户银行' }, detail: { type: 'span', serial: 9, ou: 3 } },
    { field: 'bankAccount', name: '银行账号', hide: true, show: { type: 'number', precision: null, ou: 3, placeholder: '请输入银行账号' }, detail: { type: 'span', serial: 10, ou: 3 } },
    { field: 'taxNo', name: '纳税号', hide: true, show: { type: 'number', precision: null, ou: 3, placeholder: '请输入纳税号' }, detail: { type: 'span', serial: 11, ou: 3 } },
    { field: 'invoice', name: '发票抬头', hide: true, show: { type: 'text', ou: 3, placeholder: '请输入发票抬头' }, detail: { type: 'span', serial: 12, ou: 3 } },
    { field: 'tax', name: '价格含税', hide: true, show: { type: 'radio', ou: 4, obj: 'taxStatus', value: 1 }, detail: { type: 'span', serial: 13, ou: 4 } },
  ...
  goods: [
    { field: 'listImageUrl', fixed: 'left', name: '商品图片', stype: 'image', filefield: 'file', xs: 6, sm: 6, md: 6, lg: 6, xl: 6, nameSpan: 6, show: { type: 'file', iType: 'string', ou: 4, props: { url: 'data', name: 'data' }, params: { url: 'data', name: 'data' }, action: file.state.fileUrl, headers: (typeof file.state.fileHeaders == 'function' ? file.state.fileHeaders() : file.state.fileHeaders), success: file.state.fileSuccess, listType: 'picture', style: 'width: 90%;', fileHost: file.state.fileHost, placeholder: '请上传商品列表图', node: [], rules: [{ required: true, message: '请上传商品列表图！', trigger: 'change' }] },
      detail: { type: 'span', model: 'img', ou: 4, fileHost: file.state.fileHost }
    },
    { field: 'majorImageUrl', name: '商品主图', hide: true, stype: 'image', filefield: 'file', colspan: 18, nameSpan: 6, show: { type: 'file', iType: 'string', limit: 5, ou: 4, props: { url: 'data', name: 'data' }, params: { url: 'data', name: 'data' }, action: file.state.fileUrl, headers: (typeof file.state.fileHeaders == 'function' ? file.state.fileHeaders() : file.state.fileHeaders), success: file.state.fileSuccess, listType: 'picture', style: 'width: 90%;', fileHost: file.state.fileHost, placeholder: '请上传商品主图', node: [], rules: [{ required: true, message: '请上传商品主图', trigger: 'change' }] },
      detail: { type: 'file', iType: 'string', model: 'img', isDisabled: true, ou: 4, action: file.state.fileUrl, fileHost: file.state.fileHost }
    },
    { field: 'id', hide: true, show: { type: 'hide', noShow: 1} },
    { field: 'goodsType', name: '商品类型', formatter: 'goodsType', 
    ...
    { field: 'categoryId', name: '商品分类', stype: 'mapping', mapping: 'categoryName', show: { type: 'select', ou: 2, remote: true, obj: 'categoryList', placeholder: '请选择商品分类' }, search: { type: 'text', field: 'categoryName', placeholder: '请输入商品分类' }, rules: [{ required: true, message: '请输入商品分类', trigger: 'blur' }], detail: { type: 'select', isDisabled: true, obj: 'categoryList', serial: 6, ou: 2 } },
    ...
    { field: 'goodsName', name: '商品名称', show: { type: 'text', ou: 2, placeholder: '请输入商品名称' }, rules: [{ required: true, message: '请输入商品名称', trigger: 'blur' }], detail: { type: 'span', serial: 4, ou: 2 } },
    ...
    { field: 'inventory', name: '库存数量', show: { type: 'number', ou: 5, placeholder: '请输入库存数量' }, rules: [{ required: true, message: '请输入库存数量', trigger: 'blur' }] },
